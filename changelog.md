...
- **🔧 CURRENT (2026-06-20) — analysis is being rebuilt from scratch, deterministic-first**. The old AI ad-analysis (creative analyzer + synthesizer + recommender + their UI) was **REMOVED** (see dated entries below) because it was low-value for bulk analysis. **Build is green** (typecheck 0 / lint clean / 53 tests / app boots). Work is on branch **`analysis-rework`**; git checkpoints: `7a91eb2` (initial snapshot) and `63812c2` (green after removal). **Still working:** scraping (+17 extended-capture fields), the 781 ads across 3 competitors (ClickUp=`self`, Asana, Monday.com), performance scoring + buckets, CTA-derived conversion goal, onboarding (AI company profile + competitor suggestions), swipe file. **Insights page = temporary "coming soon" placeholder.**
  - **NEXT — Part B: deterministic (zero-AI) analysis module + dashboard.** Metrics (per competitor + head-to-head): volume & live/paused; launch velocity + timeline (needs a re-scrape to populate `start_date`); media mix (image/video/carousel); DCO vs single; CTA mix (selling motion); conversion goal; copy-length bands; placement spread; creative languages; creative scaling (collation); top landing domains + offer keyword signals; advertiser context (follower count/categories); proven winners (live & >90/>180 days) + score buckets. (Verified worth building — real example output produced 2026-06-20.)
  ...
  - **BACKFILL:** the existing 781 ads have NULL in the new extended-capture columns (`start_date`/`end_date`/`video_urls`/`link_description`/`display_link`/`page_like_count`/`page_categories`/`extra_*`/etc.) until **re-scraped** (free, Playwright-only, no AI). A full re-scrape of the 3 competitors is the pending next data step.
  - **DOCS DEBT:** `docs/ai-pipeline.md` and parts of `CLAUDE.md` (commands list, AI conventions) still describe the removed AI layer — a full sweep is deferred until the rebuild lands, to avoid rewriting twice.
...
- **🟡 Known issue**: `markMissingAdsInactive` flips any previously-active ad not seen in this run to inactive — including ads beyond the `maxAds` cap. Practical effect: scraping with a small cap (say `--max-ads=10`) when 25 ads exist in the DB can incorrectly inactivate 15. Defer fix until we have a "did we actually hit end-of-library?" signal; for now use sensible caps (25/50/100 in the UI).
