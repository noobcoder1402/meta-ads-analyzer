...
- **🔧 CURRENT (2026-06-21) — analysis is being rebuilt from scratch, deterministic-first**. The old AI ad-analysis (creative analyzer + synthesizer + recommender + their UI) was **REMOVED** (see dated entries below) because it was low-value for bulk analysis. **Build is green** (typecheck 0 / lint clean / 78 tests / app boots). **Two foundations landed 2026-06-21 ahead of the analysis-module build** (see top dated entries): (1) the **snapshot model** — live/paused now trusts Meta's `is_active` directly (we scrape `active_status=all`), absence-inference (`markMissingAdsInactive`) removed, cap bug dissolved; (2) language detection swapped `tinyld` → `eld`; and `docs/meta-ads-mechanics.md` was re-sourced to **official Meta domains only** (the canonical "what Meta exposes" reference, every claim tagged VERIFIED vs INFERENCE). Work is on branch **`analysis-rework`**; git checkpoints: `7a91eb2` (initial snapshot) and `63812c2` (green after removal). **Still working:** scraping (+17 extended-capture fields; the "Scrape ads" dialog now picks one of three **scrape modes** — active / active+sample-of-paused / active+all-paused — via the ordering-safe `scrapeCompetitorByMode` orchestrator; creative-image download removed — see 2026-06-22 scrape-modes entry), **~1,846 two-pass-scraped ads across 3 competitors** (ClickUp=`self` 606, Asana 323, Monday.com 917 — Monday re-scraped 2026-06-22 after an under-capture fix took it 189→917; see that entry. ALL view, so the extended-capture date/advertiser columns are populated), CTA-derived conversion goal, onboarding (AI company profile + competitor suggestions). **The deterministic analysis module (`lib/analysis/`) feeds the cross-competitor Insights comparison page, with a sticky All ads / Active ads / Inactive ads segment filter** (neutral live-vs-not-live, no "winner/flop" framing — see the 2026-06-22 neutral-analysis entry; the Swipe File, the per-competitor detail page, and the whole scoring engine were removed in that same change). Insights also carries one clearly-fenced **external-data** block ("Company scale & regional reach" — revenue/customers/valuation/regions from public filings + tagged estimates for private brands; the only exception to the "never claim from Meta data" rule — see 2026-06-22 entry). **The Insights page now also carries a user-triggered AI "Strategic insights" narrative** (Opus reads the deterministic numbers and writes the strategic read; cached in `ai_insight_reports`; button-triggered, never on load — see 2026-06-22 AI-insights entry).
  - **Part B — deterministic (zero-AI) analysis MODULE: ✓ BUILT 2026-06-21 (`lib/analysis/`), wired into the Insights page; neutralized 2026-06-22.** Metrics (per competitor + head-to-head): volume & live/paused (snapshot model); **distinct creatives via Meta `collation_id`** (de-confounds manual-duplication vs DCO); longevity run-length tiers (LIVE only, neutral bands — NOT a quality score); active/inactive segment lens; media/structure/copy-length mix (plain shares); CTA mix (raw labels); **phrase mining** (document-frequency n-grams); placement spread; creative languages (`eld`); creative scaling (collation, display-only); top landing domains; advertiser context (followers/categories — needs re-scrape); launch velocity (needs `start_date` re-scrape); and the cross-competitor head-to-head + **self-gap**. Documented in `docs/analysis.md`. (The 2026-06-22 rework removed the winner/flop split + scoring; see that entry.)
  - **THEN — new AI analysis with FIXED ENUMS** (so it aggregates): angle · hook_type · primary_benefit · primary_pain · target_audience · offer_type · tone · visual_style + has_social_proof · cta_strength + one free-text 1-line summary. Reads caption + headline + link_description + image. (Design proposed, not yet approved/built.)
  - **BACKFILL — ✓ DONE 2026-06-21:** the old 781 ads (which had NULL extended-capture columns) were wiped and replaced by a fresh two-pass re-scrape of the 3 competitors (1,118 ads), so `start_date`/`end_date`/`video_urls`/`link_description`/`display_link`/`page_like_count`/`page_categories`/`extra_*`/etc. are now populated. (Free, Playwright-only, no AI.)
  - **DOCS DEBT — ✓ RESOLVED 2026-06-20:** swept every `.md` so docs reflect current state (not the removed AI layer). `CLAUDE.md` (commands, folder map, conventions, gotchas), `docs/ai-pipeline.md` (rewritten — surviving onboarding AI only; removed-layer design left in git, not copied into the doc), `docs/architecture.md` (diagram + lifecycle now scrape→score→dashboard), `docs/dashboard.md` (detail page = "Ads by format" matrix + ad grid; Insights = "coming soon"; removed UI noted in one line each), and a "historical snapshot" banner on `data/field-analysis.md`. See the dated entry below.
- **✓ Built (snapshot from BEFORE the 2026-06-20 AI-layer removal — the analyzer/synthesizer/recommender/scoreboard items below NO LONGER EXIST; kept for history)**: onboarding (website scrape → AI profile → self competitor), competitors grid (Tracked above Suggested) with inline Suggested section, Accept/Reject with server-truth refresh, manual add, competitor detail placeholder, `DEMO_MODE` write guards, Anthropic provider abstraction (Gemini stub), AI client with retry + friendly errors. **Meta Ad Library scraper — core in `lib/scraper/scrape-competitor.ts`, available via CLI (`pnpm scrape`) AND via dashboard `Scrape ads` button** (SSE-streamed live progress, no terminal needed). Modern DCO-aware mediaType classifier — trusts card contents (`isVideoCard` helper), not `display_format`. `upsertScrapedAd` refreshes all extraction fields on re-scrape so classifier improvements propagate without migrations. Meta page-ID resolver (`lib/scraper/resolve-meta-page.ts`) + backfill CLI (`scripts/backfill-meta-pages.ts`) for converting AI-guessed keyword URLs into verified per-page URLs. "Set Meta page" UI — paste-an-Ad-Library-URL dialog with Playwright verification, surfaced inline on any competitor card missing a verified page (closes the gap for brands the auto-resolver can't find, e.g. Monday.com). **Creative analyzer (vision) — core in `lib/ai/analyzers/analyze-creative.ts`, available via CLI (`pnpm analyze`) AND via dashboard `Analyze N ads` button** (SSE-streamed live progress). Image+caption → structured analysis (hook, angle, themes, conversion goal, brand voice) with auto-bumping `ANALYZER_VERSION` hash, prompt caching on the static block, and magic-byte image sniffing. **Performance scoring — `lib/scoring/performance-score.ts` (pure 0–100 inferred score), `lib/scoring/buckets.ts` (canonical Winner/New/Maturing/Flopped/Retired/Other predicates + tags, incl. the `Likely campaign` tag), `lib/scoring/score-ads.ts` (DB orchestrator).** Auto-runs at the end of every successful scrape (free, deterministic); persists one `performance_scores` row per ad. Read via `getScoresForCompetitor`. **Ad detail UI — the competitor detail page now surfaces both layers**: each ad card shows a bucket-colored score chip PLUS a text bucket-label pill (🏆 Winner / 🧪 Active experiment / 🌱 Maturing / ⚰️ Flopped / 📦 Retired / ○ Other) and any tags (Always-on / Paused / Likely campaign), and a **bucket count/filter bar** above the grid lets you click a bucket to see just those ads (bypassing "Active only" so Flopped/paused surface). Clicking a card opens a dialog with the full 3-signal score breakdown (longevity/placement/recency bars), bucket + tags, and the AI analysis (hook, angle, persona, tone, themes, pain points, benefits). Cards sort by Score or Longest-running. See entry below. **Competitor synthesizer — core in `lib/ai/analyzers/synthesize-competitor.ts`, available via CLI (`pnpm synthesize`) AND via dashboard `Find patterns` button**. Reads ALL of one competitor's analyzed ads, buckets them (winners/experiments/maturing/flopped/retired/other via `buckets.ts`), and writes one `competitor_syntheses` row. Hybrid design: frequency counts + always-on-winners are computed deterministically in code; one Sonnet call does only the reasoning (top hooks, recent pivots, active-experiment + abandoned clusters). User-triggered only. Surfaced as an "Ad strategy patterns" card on the competitor detail page. **Deterministic roll-ups extended 2026-06-03** with four more free (no added model tokens) fields: `creative_languages` (localization depth, detected from caption/title via `lib/lang/detect-languages.ts` using tinyld short-text detection — never the CTA), `media_mix` (image/video/carousel), `top_pain_points`/`top_benefits` (what they hammer), and `launch_velocity` (new live ads by Meta start_date, last 14/30d). Surfaced in the synthesis panel (stat strip + language chips + pain/benefit rows). **GTM recommender (engine) — core in `lib/ai/analyzers/generate-recommendations.ts`, available via CLI (`pnpm recommend`)**. Cross-competitor gap analysis: compares the `self` synthesis (or `company.md` positioning if the user has no ads) against ALL competitor syntheses along four dimensions (angle, selling motion / CTA, brand voice, always-on patterns) → 5–10 prioritized `recommendations` rows in one Sonnet call. Grounded by an "evidence catalog" of real competitor ads (top 12 by score each, cited by `library_id`); invented IDs are validated out. **Replace-on-run**: each run fully replaces the previous set (no "done"/actioned state, no archive). See entry below. **GTM recommender UI — the Insights page (`app/insights/page.tsx`) renders the saved recommendations** (priority-sorted cards: priority badge, title, rationale, and cited evidence `library_id`s linked to the Meta Ad Library) with a `Generate recommendations` / `Regenerate` button wired to a new `DEMO_MODE`-guarded `POST /api/recommendations/generate` route wrapping `generateRecommendations()` (target 5–10, fixed). Plain JSON (single Sonnet call), mirrors the synthesize route + `synthesis-panel` client pattern. No "done" toggle, no Archive tab — replace-on-run. See entry below. **Competitor scoreboard (Insights)** — a deterministic, zero-AI side-by-side table (`app/insights/_components/competitor-scoreboard.tsx`, fed by `getSynthesesForActiveCompetitors`) of every active competitor with `self` pinned/highlighted; reads saved `competitor_syntheses` roll-ups (ads, media mix, launch velocity, languages, top CTA/voice/angle, top pain points/benefits) with a "synthesized {age}" staleness stamp, a muted "not analyzed yet" row for un-synthesized competitors, and an exact-match shared-highlight on overlapping pain points/benefits. See entry below. **Scraping** — the "Scrape ads" button owns volume + live/paused + scoring and defaults to **All countries** (`country=ALL`: widest volume + authoritative `is_active`), with **Specific country** for single-market investigation. (The per-country "Map markets" / footprint feature was **removed 2026-06-04** — Meta exposes no reliable per-ad geography; see entry below.) Shared constants in pure `lib/markets.ts` (`ALL_COUNTRIES` sentinel + `COUNTRY_OPTIONS` picker list). **Swipe File page (`app/swipe-file/page.tsx`)** — cross-brand gallery of every scored ad (all competitors + the user's own `self` brand, brand-labeled), grouped into 🏆 Winners / 🧪 New & testing / ⚰️ Flopped via the canonical `buckets.ts` predicates. Angle filter pills, Score/Longest-running/Recently-added sort, "Show dropped ads" toggle (off by default). Pure read-only (three cross-competitor queries: `getSwipeFileAds`, `getAllScores`, `getAllAnalyses`); reuses the existing `AdDetailDialog` drawer. See entry below.
- **✓ Resolved — scraper under-capture root cause found & fixed 2026-05-31**: every scrape was silently capping at the ~30 ads in the initial SSR HTML. The earlier wheel-event change was a red herring — instrumentation proved the page scrolled fine AND Meta fired paginated GraphQL responses (350+ ads in <1 min), but the response listener's content-type allowlist (`json`/`ndjson`/`javascript`) **dropped every paginated response because Meta labels them `text/html`**. Fix: gate the listener on URL first, then accept `text/html` too (`lib/scraper/scrape-competitor.ts`). Validated live — ClickUp US went 30 → 200 (capped). Full 3-competitor geo re-sweep at 200/market running. See entry below.
- **⏭ Next up**: `pnpm refresh:all` bulk re-scrape (not yet implemented). ~~Add an `awareness` conversion goal~~ **DONE 2026-06-03** — but solved differently than originally planned: instead of teaching the model a new enum value, the conversion goal is now **derived deterministically from the Meta CTA** (no AI), which made the `awareness` add a free `pnpm backfill:goals` rather than a paid re-analyze. See entry below.
- **⚠️ Temporary**: competitor suggester is on `claude-sonnet-4-6` instead of `claude-haiku-4-5` because Anthropic's Haiku pool was throwing sustained 529s on 2026-05-22. Single-line swap in `lib/ai/analyzers/suggest-competitors.ts:46` — revert to `"haiku"` once capacity stabilizes (Sonnet is ~10× the cost per call).
- **✗ Superseded — the entire scoring engine was REMOVED 2026-06-22** (see top dated entry). The three-signal note below is kept for history only; `lib/scoring/` and the `performance_scores` table no longer exist.
- **✓ Resolved (scoring) — re-weighted to 3 signals 2026-05-31**: scoring used to have *four* signals, two of which were structurally dead. **Placements was fixed 2026-05-30** (scraper now reads `publisher_platform`). **Variants is now removed entirely** rather than left dead — Meta exposes no reliable variant count (`cards[]` conflates carousel slides with DCO variants; `collation_count` is null/1 in the raw data, confirmed against a real scrape dump). Its 20 points were redistributed → **longevity 60 / placement 20 / recency 20**, so the 0-100 scale is honest. The degenerate `hidden-gem` tag (keyed on `variant_count == 1`, so it fired on every 60+ ad) was dropped too. `variant_count`/`variant_pts` columns kept as unused legacy. See entry below + `docs/scoring.md`.
- **✓ Resolved — active/paused was wrong for global advertisers; `ALL` made the default scrape 2026-06-03**: a combined geo-sweep was flipping global/Advantage+ brands to 100% paused (Monday.com read **0 active of 827 ads**). Root cause: the sweep looped countries first, appended `ALL` last, and the union used first-occurrence-wins — so the paused per-country `is_active` locked in and the active `ALL` value was discarded. Fix: made `ALL` the **default scrape** (authoritative live/paused). The "Map markets" footprint sweep that originally carried the other half of this fix was **removed entirely 2026-06-04** (see top entry) — `ALL`-default is the part that survived, and it fully resolves the active/paused bug on its own.
- **🟢 Action needed (data hygiene) — a re-analyze is now PENDING**: on 2026-06-04 `pnpm clean:ads` deleted **976 paused + un-analyzed ads** (Monday 664, ClickUp 289, Asana 23), leaving **781 ads** (280 active + 501 analyzed-paused). Adding the `ai-powered` angle bumped `ANALYZER_VERSION`, so **all 781 remaining ads now read as "needs analysis"** (their stored analyses predate the new angle). The user **deferred the paid re-analyze** — until it runs, the new `ai-powered` angle won't appear and the raw-CTA "selling motion" (`dominant_ctas`) stays blank on existing syntheses. Full refresh = `pnpm analyze` (paid, ~$3–5 on Haiku for 781) → `pnpm synthesize` each brand → optionally `pnpm recommend`, in that order (synth/recs are replace-on-run). (b) Several earlier scrape-time fixes (paused-ad `daysActive`, `placements`, carousel/DCO `media_type`, `title`/`display_format`/`collation_*`/`contains_ai_media`) only propagate on re-scrape; competitors beyond ClickUp/Asana/Monday still need a re-scrape. No migration needed — just "Scrape ads".
- **✗ Superseded — geographic insights (shipped 2026-05-31) were REMOVED 2026-06-04**: the scrape dialog briefly offered a 3-way market picker (Specific country / All countries / Geo sweep) plus a Markets card, per-ad country badges, and an Insights Market-gaps panel. All of it was torn out (see top entry) — Meta exposes no reliable per-ad geography, so the footprint signal was thin and confusing. The scrape dialog is back to 2 modes (All countries / Specific country).
- **✓ Resolved — snapshot model, `markMissingAdsInactive` removed 2026-06-21**: we no longer infer "paused" from an ad's absence. Because we scrape `active_status=all`, Meta reports each ad's real `is_active` directly (measured: ~93% of our "paused" ads are Meta's own flag; only ~7% are true disappearances). An ad not found in a later scrape is left as-is; "live" is derived as "present in the latest scrape". This **dissolved the old `--max-ads` cap bug** (a shallow scrape can no longer wrongly inactivate ads past the cap, since nothing flips on absence). See entry below.
